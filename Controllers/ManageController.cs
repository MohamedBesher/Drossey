...
using Drossey.Models.ManageViewModels;
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            if (model.CountryId != 0)
                model.Grades = new SelectList(_unitOfWork.GradeRepository.Filter(u => u.CountryId == model.CountryId && u.IsPuplished), "Id", "Name");

            //ViewData["PhotoUrl"] = user.PhotoUrl;
            //ViewData["userName"] = user.FirstName + ' ' + user.LastName;
            //ViewData["Balance"] = user.Balance;
            ViewData["Active"] = "editprofile";
        ...
        public async Task<IActionResult> Index(IndexViewModel model)
        {
            ...
            try
            {
                ...
                if (!model.PhotoUrl.Equals(user.PhotoUrl))
                {
                    user.PhotoUrl = "/uploads/" + model.PhotoUrl;
                }
                ...
                if (!result.Succeeded)
                {
                    throw new ApplicationException($"Unexpected error occurred setting update for user with ID '{user.Id}'.");
        ...
        {
           var user = _unitOfWork.UserRepository.GetOneUser(_userManager.GetUserId(User));
            //var user = await _userManager.GetUserAsync(User);
            if (user == null)
            {
                throw new ApplicationException($"Unable to load user with ID '{_userManager.GetUserId(User)}'.");
