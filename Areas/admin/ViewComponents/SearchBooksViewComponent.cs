...
namespace Drossey.Areas.admin.ViewComponents
{
    public class SearchBooksViewComponent : ViewComponent
    {
        ...
        public SearchBooksViewComponent(IUnitOfWorkAsync unitOfWork, IMapper mapper)
        {
        ...
        public async Task<IViewComponentResult> InvokeAsync(int? page, int pageSize,string keyword = "", long countryId=0, long gradeId=0, long termId=0, long subjectId = 0)
        {
            ...
            IQueryable<BookDto> books = _unitOfWork.BookRepository
                .SearchBooks(page, pageSize, keyword, countryId, gradeId, termId, subjectId);
            ViewBag.ResultCount = books.Count();
            int result = (books.Count() / pageSize) + (books.Count() % pageSize > 0 ? 1 : 0);
            if (page > 1 && result < page)
            {
                ViewBag.Page = page - 1;
                var list = await PaginatedList<BookDto>.CreateAsync(books, page - 1 ?? 1, pageSize);
                return View(list);
            }
            else
            {
