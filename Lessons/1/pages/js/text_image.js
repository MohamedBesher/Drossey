...
$(document).ready(function() {  
    // this key code to link with player and load json 
       /* loadScript('../Player/js/redundant.js', function (a, b) {}).onload = function () {
        loadScript('../Player/lessons/' + lessonID + '/media/text_image.js', function (a, b) {}).onload = function () {*/
...
//    });
    ...
    for(var s=0; s<mydata.dynamicData.items.length;s++){
        $('.text ol').append('<li>'+mydata.dynamicData.items[s]+'</li>');
    }
    
    $('.img-pos').attr('src',mydata.dynamicData.image_source);
//    
