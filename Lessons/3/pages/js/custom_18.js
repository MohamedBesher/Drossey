...
//    localStorage.setItem("cardsCount", DG.length);
    ...
    for (var i=0;i<DG.length;i++){
        ...
        cardsSrc.push({x:i+1, y:DG[i].img_src ,z:DG[i].desc,height:DG[i].height,width:DG[i].width});
...
    $('<div  class="grid-md-5 grid-sm-4 grid-xs-2"><div class="sec_item" style = "height:'+ cardsSrc[i].height +';width:'+cardsSrc[i].width+'"><div class="sec_img"><img src="' + cardsSrc[i].y + '" alt=""/></div><div class="sec_num"></div><p class="sec_desc">' + cardsSrc[i].z  + '</p></div></div>').data( 'number', cardsSrc[i].x ).appendTo( '.section_items' ).droppable( {
      accept: '.circle_num li',
      hoverClass: 'hovered',
      drop: handleCardDrop
    } );}
