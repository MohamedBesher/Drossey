...
function start(pageNum) {
    ...
    var lesson_id=  sessionStorage.getItem('lessonId');
              ...
              loadScript("../Data/Lessons/"+lesson_id+"/"+jsonFile+".js",function (a, b) {}).onload = function ()
        {
            ...
            //narration when click on indecator image
        //    $(".carousel-indicators li").bind('click', function (e) {
               
              /*  $(".carousel-indicators li").click(function (e) {
                
                 //play tab narration
                if(pages[pageNum].data_gallary[Number( $(this).find(".active").attr("data-slide-to"))].sub_narration != undefined && pages[pageNum].data_gallary[Number( $(this).find(".active").attr("data-slide-to"))].sub_narration != ''){
                 if(audioTap!=undefined){ if(audioTap[0]!=undefined  )audioTap[0].pause()}
                    
                   playTapAudio(pages[pageNum].data_gallary[Number( $(this).find(".active").attr("data-slide-to"))].sub_narration);
...
function setGallery(location, dataG,gType) {
    location.html('<div id="carousel-images" class="carousel slide" data-interval="false" data-ride="carousel"><!-- Indicators --><!-- Wrapper for slides --><div class="carousel-inner" role="listbox"></div><!--end carousel-inner--><ol class="carousel-indicators"></ol></div><!--end carousel-->');
    var innertHTML, className;
    for (var i=0;i<dataG.length;i++) {
          if (gType=='vertical'){
           // this code fpr vertical slide page 16
      // innertHTML='<div class="row"><div class="col-md-6 col-xs-12"><img src="'+dataG[i].main_src+'" class="img_shadow" alt="..."></div><!--end col-md-9 col-xs-12--><div class="col-md-6 col-xs-12"><div class="slide_content"><h3 class="title_font">'+dataG[i].title+'</h3><p>'+dataG[i].desc+'</p></div><!--end slide_content--></div><!--end col-md-3 col-xs-12--><div class="clearfix"></div></div><!--end row-->'
        
            innertHTML='<div class="row"><div class="col-md-6 col-xs-12"><img src="'+dataG[i].main_src+'" class="img_shadow" alt="..."  style="'+dataG[i].img_style+'"></div><div class="slide_content col-md-6 col-xs-12"><h3 class="title_font" style="'+dataG[i].style+'">'+dataG[i].title+'</h3><p>'+dataG[i].desc+'</p></div><!--end slide_content--><div class="clearfix"></div></div><!--end row-->'
        }else {
        ...
        //carousel html code 
 $('.carousel-indicators').append('<li  onclick="loadAudio('+i+')" data-target="#carousel-images" data-slide-to="'+i+'" '+className+'><a href="#"><img  src="'+dataG[i].icon_src+'" alt="" width="170px" height="120px" /></a></li>')
...
{
     //play tab narration
                if(pages[currentPage].data_gallary[id].sub_narration != undefined && pages[currentPage].data_gallary[id].sub_narration != ''){
                 if(audioTap!=undefined){ if(audioTap[0]!=undefined  )audioTap[0].pause()}
                    
                   playTapAudio(pages[currentPage].data_gallary[id].sub_narration);
