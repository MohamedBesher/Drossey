...
//**************change here only *******************/
        ...
        sessionStorage.setItem('lessonId',"Sci/G07_T02_U04_Ch07_L01");
        sessionStorage.setItem('Title',"الغلاف الجوى والطقس");
//**************change here only************** */

        lesson_id = sessionStorage.getItem('lessonId');
        MyObject="";
      
        loadScript("/mylessons/2/Data/Lessons/"+lesson_id+"/lesson_key.js",function (a, b) {}).onload = function ()
        {
            Page = lesson_data;
       ...
       function ViewBookmark(lesson_id,index){
         ...
         */
                ...
                if(Page[index].source == "text_image3_animation")
                    drawObjective(index);
                     ...
                     $('.site_contents').html($('<div><object data="2/pages/'+Page[index].source+'.html" style="width:100%;  min-height:'+Pageheight+'px"></object></div>'));
            ...
            {
                 $('.site_contents').html('<div  class="loader" style="margin:300px auto"></div>');
               // $('.site_contents').html($('<div class="video_cont"> <video class="main_content" autoplay controls="controls" style="display: block;"><source src="Data/Lessons/'+lesson_id+'/video/'+ Page[index].source + '.ogv" type="video/ogg"/><source src="Data/Lessons/'+lesson_id+'/video/'+ Page[index].source + '.mp4" type="video/mp4"/></video><div>'));
                 ...
                 loadScript("/mylessons/2/Data/Lessons/" + lesson_id +"/video/" + Page[index].source + ".js",function (a, b) {}).onload = function (){
        ...
        if(Page[index].audio != ''){
            ...
            Page[index].audio=Page[index].audio.replace("../lesson_player",".../..");
            $('.container-fluid .row div:nth-child(1)').html('<audio class="audio_player" width="100%" height="100%" controls autoplay><source type="audio/ogg" src="'+lesson_data[index].audio.replace("../lesson_player",".../..")+'.ogg"/><source type="audio/ogg" src="'+lesson_data[index].audio.replace("../lesson_player",".../..")+'.mp3"/></audio>');
...
function drawObjective(index)
{
    ...
    //var lesson_id=  sessionStorage.getItem('lessonId');
     ...
     loadScript("/mylessons/2/Data/Lessons/"+lesson_id+"/"+jsonFile+".js",function (a, b) {}).onload = function ()
        {
            start();
        };
}
