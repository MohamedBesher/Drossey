...
 
    $.each(randomAnswerData,function(index,value){
       // console.log(value.x) 
        ...
        $('<a href="#">'+value.co.answer+'</a>').data( 'number', value.x ).appendTo("nav.temp_22_nav").draggable({
    ...
    if($.trim(value.direction)=="left"){className='pos_left'}
           $('<span style="top:'+value.y+'px;right:'+value.x+'px;width:'+value.width+'px;height:'+value.height+'px;font-size:'+value.font+'px" class=" drop-text '+className+'">------</span>').data( 'number', index ).appendTo(".science_temp").droppable( {     
...
function loadHTMLTags(pageNum) {
   ...
   if(pages[pageNum].article!=undefined){
    ...
    $('<div class="row container"> <div class="col-md-3 col-sm-3 col-xs-12" style="font-size:'+pages[pageNum].col_font+'px">  <nav class="temp_22_nav row" >  </nav></div><!--end col-xs-12--> <div class="col-md-9 col-sm-9 col-xs-12"> <div class="blue_bg"> <div class="science_temp"> <img src="'+pages[pageNum].main_img+'" style="'+pages[pageNum].style+'"/></div> </div><!--end blue_bg--> </div><!--end col-xs-12--> </div><!--end row-->').appendTo('.contents')
    setCorDeclar()
