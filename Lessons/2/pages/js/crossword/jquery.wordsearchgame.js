...
var Visualizer = {
    ...
    signalWordFound : function (w) {
		...
		$(w).css("background",'#3169AA').animate({"opacity": 'hide'},1000,"linear",
					 function () {
						 $(w).css("background",'#E2F7FF')
...
function Grid() {
   ...
   var alphabet = "ذدجحخهعغفقثصضشسيبلاتنمكطظزوةىرؤئ".split(""); 
    for (var i=0;i<this.size();i++) {
        for (var j=0;j<this.size();j++) {
            
            if (this.cells[i][j].isUnwritten()) {
                this.cells[i][j].value = alphabet[Math.floor(Math.random()*32)]
                   //String.fromCharCode(Math.floor(193+Math.random()*26));
// console.log("ar  "+alphabet[Math.floor(Math.random()*38)])
...
var GameWidgetHelper = {
    ...
    renderGame : function(container, model) {
        ...
        var words = "<div id='rf-wordcontainer'><ul class='cross_menu'>"
        for(var x=0; x<wordsDetails.length; x++)
        {
            //console.log("ui "+this.isPlaced)
            words += '<li class="rf-p'+this.isPlaced+'">'+wordsDetails[x].definition+'</li>';
        }
        words += "</ul></div>";
...
$(container).append(puzzleGrid);
	...
	signalWordFound : function(idx) {
		...
		if (!jQuery(w).hasClass('')){
			Visualizer.signalWordFound(w);
			currentWord++;
			if (typeof encontrePalabra == "function"){
					encontrePalabra({id:idx,feed_back:wordsDetails[idx].feed_back,narration:wordsDetails[idx].sub_narration });
               // console.log(idx)
				}
