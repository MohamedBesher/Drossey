...
$('.close_msg').click(function() {
    ...
    //read json file
       ...
       if(data.title != undefined  && data.inside_title != undefined)
    ...
    $(".w3-teal").click(function(){
        ...
        if($(".selected").length == 0)
            ...
            $(".choice:contains('"+data.questions[questionNumber].correctChoise+"')").addClass("correct");
        ...
        else if( $(".selected").text() == data.questions[questionNumber].correctChoise )
        {
            //show popup of result ..
            correct_answer++;
        ...
        if(questionNumber == data.questions.length-1 )
            {
                //show result
                  //show popup of result ..
             $("#result_here").empty().append("<p class='alert alert-success'>لقد أجبت "+correct_answer+" من "+data.questions.length+" </p>").append('<button type="button" class="btn btn-primary" data-dismiss="modal">إغلاق</button>');
...
function Drawquestion(questionNumber)
{
    ...
    $(data.questions[questionNumber]).each(function(i,json){  
