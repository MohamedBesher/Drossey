    ...
    if (queBackUp[c] == undefined || quizData[c].type == "essay") {
        ...
        switch (quizData[c].type) {
            case "essay":
                ...
                $('#text_modal .frame_text').html(quizData[c].rightfeedback)
                
                $('<textarea class="form-control" rows="4" placeholder="اكتب اجابتك هنا  ..."></textarea>').appendTo("#quiz_body")
...
//                        console.log(reportArr[index].degree)
                ...
                reportArr[index] = {
                    //******عملت كومنت حتى اتاكد من المطلوب
                    "coAnwser": "--",
                   //"coAnwser": quizData[index].co_answer,
                    "stAnwser": "--",
                    "degree": "لم يتم الحل "
                }
...
//                        console.log(quiz['report'].singleChoiceTxt)
        ...
        //**************
        sessionStorage.setItem("quizDegree", JSON.stringify(reportArr));
        ...
        $.each(reportArr, function (index, value) {
            if (value != undefined) {
                ...
                //save all report  value.coAnwser
