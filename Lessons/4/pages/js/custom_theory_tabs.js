...
var setSlider = function(imgs) {
    ...
    $.each(imgs,function(index,value){
        ...
        imgTemp+='<div class="item '+className+'"><img src="'+value.img+'" ></div>';
        className=""
    })
...
function init(pageNum) {
    ...
    $(' <div class="temp28_head"><h2>'+pages[pageNum].main_tobic.title+'</h2> <h4>'+pages[pageNum].main_tobic.text+'</h4></div><!--end temp28_head-->').appendTo('.contents')
    }
    $('<div class="temp_28_tabs"><div class="row"> <div class="col-md-3 col-sm-4 col-xs-12"> <ul id="menu" class="nav nav-tabs " role="tablist"></ul> </div><!--end col-xs-12--><div class="col-md-9 col-sm-8 col-xs-12 temp28_tab_line"> <div id="desc" class="tab-content"></div><!--end tab-content--></div><!--end col-xs-12--></div><!--end row--></div><!--end temp_28_tabs-->').appendTo('.contents')
...
function loadTemplateBody(){
    var setIMediaIsFound=function(value){
        ...
        if( value.img != undefined  ){
            media='<div id="media_img"  '+tempClass+'><img src="'+value.img+'" style="'+value.style+'" /></div>'
        ...
        if (value.desc != undefined && value.desc != "" && media != "") {
            srt = '<div class="col-xs-12 col-md-6 col-sm-6"><h4>' + value.desc + '</h4></div><div class="col-xs-12 col-md-6 col-sm-6">' + media + '</div><div class="clearfix"> </div>'
        } else if (value.desc != undefined && value.desc != "" && media == "") {
            srt = '<h4>' + value.desc + '</h4>'
        } else {
