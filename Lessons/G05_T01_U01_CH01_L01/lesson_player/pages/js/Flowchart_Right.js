...
$(function(){
 ...
 /*   loadScript('../Player/js/redundant.js', function (a, b) {}).onload = function () {
        loadScript('../Player/lessons/' + lessonID + '/media/Flowchart_Right.js', function (a, b) {}).onload = function () { */
...
function loadTabs(){
    //flow chart title
    $('.temp_255_chart h1').html(mydata.main_title); 
    //load tabs buttons
    for(var n=0; n<mydata.dynamicData.length; n++){
        $('.temp_255_chart ul').append('<li><a href="#">'+mydata.dynamicData[n].title+'</a></li>');
    }
...
function loadTabContent(num){
    if(mydata.dynamicData[num].type=='text'){
        $('.blue_bg .row').html('<div class="col-md-12 col-sm-12 col-xs-12"><h4 class="text-line">'+mydata.dynamicData[num].paragraph+'</h4></div>');
    
    }else if(mydata.dynamicData[num].type=='textWithImage'){
        $('.blue_bg .row').html('<div class="col-md-6 col-sm-6 col-xs-6"><h4 class="text-line">'+mydata.dynamicData[num].paragraph+'</h4></div><div class="col-md-6 col-sm-6 col-xs-6 text-center" ><img src="'+mydata.dynamicData[num].mediaSource+'"/></div>');
    
    }else if(mydata.dynamicData[num].type=='textWithVideo'){
        $('.blue_bg .row').html('<div class="col-md-6 col-sm-6 col-xs-6"><h4 class="text-line">'+mydata.dynamicData[num].paragraph+'</h4></div><div class="col-md-6 col-sm-6 col-xs-6 text-center" ><video autoplay controls style="width:100%"><source src="'+mydata.dynamicData[num].mediaSource+'.ogv" type="video/ogg"/><source src="'+mydata.dynamicData[num].mediaSource+'.mp4" type="video/mp4"/></video></div>');
    
    }else if(mydata.dynamicData[num].type=='TextWithSlider'){
        $('.blue_bg .row').html('<div class="col-md-6 col-sm-6 col-xs-6"><h4 class="text-line">'+mydata.dynamicData[num].paragraph+'</h4></div><div class="col-md-6 col-sm-6 col-xs-6 text-center" ><div class="media_cont"></div></div>');
        ...
        $('.blue_bg .row').html('<div class="col-md-12 col-sm-12 col-xs-12 text-center" ><img src="'+mydata.dynamicData[num].mediaSource+'"/></div>');
    }    
}
