...
function load_tooltip_content(){
    ...
    if(mydata.mediaType=='text'){
        ...
        $('.contents .row').append('<div class="col-xs-12 col-md-6 col-sm-8"><article><p></p></article></div><div class="col-xs-12 col-md-6 col-sm-4"><div class="media_cont"><img src="'+mydata.mediaSource+'" alt=""/></div></div>');
    ...
    $('.badge').mouseover(function(){
        ...
        for(var x=0; x<mydata.dynamicData.length;x++){
            if(mydata.dynamicData[x].word == $(this).text() && mydata.dynamicData[x].sub_narration != ''){
                narration=document.getElementById('narration');
                canPlayMP3 = (typeof narration.canPlayType === "function" && narration.canPlayType("audio/mpeg") !== "");
                narration.src=canPlayMP3?mydata.dynamicData[x].sub_narration+'.ogg':mydata.dynamicData[x].sub_narration+'.mp3';
                narration.load();
                narration.play();
                break;
            }else{
