...
$(document).ready(function() { 
    // this key code to link with player and load json 
        ...
        loadScript('../Player/lessons/' + lessonID + '/media/Interactive_Images2.js', function (a, b) {}).onload = function () {*/
    ...
    $('.image_thumb a img').click(function(){
        ...
        if($(this).parent().parent().children('.img_desc').hasClass('opend')){
            ...
            if(mydata.dynamicData[$(this).attr('id')].sub_narration != undefined && mydata.dynamicData[$(this).attr('id')].sub_narration != ''){
                narration=document.getElementById('narration');
                canPlayMP3 = (typeof narration.canPlayType === "function" && narration.canPlayType("audio/mpeg") !== "");
                narration.src=canPlayMP3?mydata.dynamicData[$(this).attr('id')].sub_narration+'.ogg':mydata.dynamicData[$(this).attr('id')].sub_narration+'.mp3';
                narration.load();
                narration.play();
            }else{
                narration.pause();
...
function loadOne(){
    for(var x1=0;x1<mydata.dynamicData.length;x1++){
        $('.contents .row').append('<div class="col-xs-12"><div class="image_thumb wow zoomIn"><a href="javascript:void(0)"><img id="'+x1+'" src="'+mydata.dynamicData[x1].image+'"/></a><div class="img_desc"><p class="title_font">'+mydata.dynamicData[x1].text+'</p></div></div></div>');
    }    
}

function loadTwo(){
    for(var x1=0;x1<mydata.dynamicData.length;x1++){
        $('.contents .row').append('<div class="col-xs-6 col-sm-6"><div class="image_thumb wow zoomIn"><a href="javascript:void(0)"><img id="'+x1+'" src="'+mydata.dynamicData[x1].image+'"/></a><div class="img_desc"><p class="title_font">'+mydata.dynamicData[x1].text+'</p></div></div></div>');
    }
}
...
function loadThrees(){
    for(var x1=0;x1<mydata.dynamicData.length;x1++){
        ...
        $('.contents .row').append('<div class="col-xs-4 col-md-4 col-sm-4"><div class="image_thumb wow zoomIn"><a href="javascript:void(0)"><img id="'+x1+'" src="'+mydata.dynamicData[x1].image+'"/></a><div class="img_desc"><p class="title_font">'+mydata.dynamicData[x1].text+'</p></div></div></div>');
    }
}
...
function loadFours(){
    for(var x1=0;x1<mydata.dynamicData.length;x1++){
        ...
        $('.contents .row').append('<div class="col-xs-3 col-md-3 col-sm-3"><div class="image_thumb wow zoomIn"><a href="javascript:void(0)"><img id="'+x1+'" src="'+mydata.dynamicData[x1].image+'"/></a><div class="img_desc"><p class="title_font">'+mydata.dynamicData[x1].text+'</p></div></div></div>');
    }
}